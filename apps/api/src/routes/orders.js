...
export async function orderRoutes(fastify) {
    fastify.post('/r/:slug/orders', async (request, reply) => {
        ...
        try {
            ...
            const result = await db.$transaction(async (tx) => {
                ...
                if (dbItems.length !== menuItemIds.length) {
                    const error = new Error('Some items do not belong to this restaurant or do not exist');
                    error.statusCode = 422;
                    throw error;
                }
                const unavailableItems = dbItems.filter(i => !i.isAvailable);
                if (unavailableItems.length > 0) {
                    const error = new Error('Some items are currently not available');
                    error.statusCode = 409;
                    throw error;
                }
    ...
    fastify.get('/r/:slug/orders/:orderNumber', async (request, reply) => {
        const { slug, orderNumber } = request.params;
        try {
...
function formatOrderForWhatsApp(order, currency) {
    const itemsList = order.items
        .map((item) => `- ${item.quantity}x ${item.itemName} (${currency} ${item.itemPrice})`)
        .join('\n');
